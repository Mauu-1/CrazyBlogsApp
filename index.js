...
app.get("/blog", (req, res)=>{
    ...
    //     res.render("blog.ejs",{ activePage: 'blog',
    //     // title: writtenTitle,
    //     message: "No posts Yet!",
    //     // postMade:  false
    // });
    // }else if(totalPosts.length >=1 ){
    //     res.render("blog.ejs",{ activePage: 'blog',
    //     postMade:  true
    //     });
    // }
    
    let postMade = totalPosts.length > 0;
    let d = new Date();
    res.render("blog.ejs", {activePage: 'blog',
            postMade: postMade,
            posts: totalPosts,
            title: totalPosts[totalPosts.length -1],
...
app.get("/createPost", (req, res)=>{
    res.render("createPost.ejs", { activePage: 'createPost' });
})
// app.get("/editPost", (req, res)=>{
//   const postId = req.params.postId;
//   console.log(postId);
//   res.render("editPost.ejs", {activePage: 'blog'});
// })
app.post("/blog", (req, res) => {
    ...
    const id = uid();
    let d = new Date();
    // let number = totalPosts.length - 1;
    totalPosts.unshift({titleText: req.body.titleText,
    postInfo: req.body.postInfo,
    htmlContent1: `<div class="solution_card">
  ...
  uniqueId: id
    ...
    console.log(id);
    console.log("post id= "+totalPosts[totalPosts.length -1].uniqueId);
    res.redirect("/blog");
    let numero = totalPosts.length
    })
app.post("/delete-post", (req, res) => {
   const { postId } = req.body;
   const postIndex = totalPosts.findIndex(post => post.uniqueId === postId);
  if (postIndex > -1) {
      totalPosts.splice(postIndex, 1);
...
app.post("/editPost", (req, res) => {
  const { postId } = req.body;
  const postIndex = totalPosts.findIndex(post => post.uniqueId === postId);
  const  currentPost = totalPosts[postIndex];
...
app.post("/edition", (req, res) => {
  const {titleText, postInfo, uniqueId} = req.body;
  const postIndex = totalPosts.findIndex(post => post.uniqueId === uniqueId);
  console.log(postIndex);
  console.log(req.body);
  totalPosts[postIndex].titleText = titleText;
  totalPosts[postIndex].postInfo = postInfo;
  res.redirect("/blog");
});
